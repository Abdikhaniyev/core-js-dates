...
function getNextFriday(date) {
  ...
  nextFriday.setDate(nextFriday.getDate() + 5 - currentDay);
  return nextFriday;
}
...
function getWeekNumberByDate(date) {
  let day = new Date(date.getUTCFullYear(), 0, 1);
  let count = 0;
  if (day.getUTCDay() !== 1) {
    day.setUTCDate(1 + ((1 - day.getUTCDay() + 7) % 7));
    count += 1;
  }
  while (day.getTime() < date.getTime()) {
    day = new Date(day.getTime() + 7 * 24 * 3600000);
    count += 1;
  }
  if (
  ...
  while (start <= end) {
    if (isWorkDay) {
      for (let i = 0; i < countWorkDays; i += 1) {
        if (start <= end) {
          result.push(
            start.toJSON().slice(0, 10).split('-').reverse().join('-')
          );
